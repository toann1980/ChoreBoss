# CORRECTED: QUICK_START_MEMORY_SIGNALING.md (With WSL2 Fix)

**For:** Toan (on NUC + Alienware)  
**Time:** ~15 minutes total  
**Status:** Updated with WSL2 mount fix

---

## PART 0: WSL2 SAMBA MOUNT (Required First!)
...
# Should see: TO_NOVA.md, TO_KIRA.md, DECISIONS.md, NOVA.md, etc.
```

If mount fails, see IMMEDIATE_FIX_WSL2_SAMBA.md for troubleshooting.

---

## PART 1: NUC SETUP (10 min)
...
## PART 2: KIRA SETUP ON ALIENWARE WSL2 (5 min)

### Step 2.1: Create Kira's Cron Script
```bash
# On WSL2 (with /mnt/memory-sync mounted):
cat > ~/kira-message-check.sh << 'EOF'
#!/bin/bash
# Check TO_KIRA.md every 2 minutes for Nova's messages

MESSAGE_FILE="/mnt/memory-sync/TO_KIRA.md"  # ← UPDATED PATH
...
**On Alienware WSL2:**
- [ ] /mnt/memory-sync mounted (`ls /mnt/memory-sync/`)
...
### Samba mount not working
See: IMMEDIATE_FIX_WSL2_SAMBA.md

### Kira's cron not triggering
```bash
# Check cron logs
...
# Fix if needed
chmod 666 /mnt/memory-sync/TO_NOVA.md
chmod 666 /mnt/memory-sync/TO_KIRA.md
```
...
**Confirm:** Run this to show setup status
```bash
echo "=== MEMORY SIGNALING STATUS ===" && \
echo "inotifywait: $(which inotifywait)" && \
echo "nova-watch.sh: $(ls -la ~/.openclaw/workspace/nova-watch.sh 2>/dev/null | awk '{print $NF}')" && \
echo "Running: $(ps aux | grep -v grep | grep nova-watch | wc -l) process(es)" && \
echo "" && \
echo "TO_NOVA.md: $(stat -c%s /mnt/memory-sync/TO_NOVA.md 2>/dev/null) bytes" && \
echo "TO_KIRA.md: $(stat -c%s /mnt/memory-sync/TO_KIRA.md 2>/dev/null) bytes"
```
...
```
✅ Samba share mounted on WSL2
✅ Nova's inotifywait running
...
**Updated:** 2026-04-21 07:32 UTC (Added WSL2 Samba mount fix)
