# QUICK START: Memory Signaling Setup (Copy-Paste Ready)

**For:** Toan (on NUC + Alienware)  
**Time:** ~15 minutes total  
**After:** Run these, Phase 0 can begin
...
# Check message:
tail -20 /srv/memory-sync/TO_KIRA.md
...
# (See MEMORY_SIGNALING_SETUP.md for advanced setup)
```
...
# Check file access
ls -la /srv/memory-sync/TO_KIRA.md
stat /srv/memory-sync/TO_KIRA.md
```
...
echo "" && \
echo "TO_NOVA.md: $(stat -c%s /srv/memory-sync/TO_NOVA.md 2>/dev/null) bytes" && \
echo "TO_KIRA.md: $(stat -c%s /srv/memory-sync/TO_KIRA.md 2>/dev/null) bytes"
```
